...
<div class="wrap atlr-wrap">
	<h1>
		<?php esc_html_e( 'Atlas Returns', 'atlas-returns' ); ?>
	...
	<?php if ( ! $is_pro && ! $can_create ) : ?>
		<div class="atlr-limit-notice atlr-limit-exceeded">
			<p>
				<strong><?php esc_html_e( 'Monthly limit reached!', 'atlas-returns' ); ?></strong>
				<?php
				printf(
					/* translators: %s: upgrade URL */
					wp_kses(
						__( '<a href="%s">Upgrade to Pro</a> for unlimited returns.', 'atlas-returns' ),
	...
	<div class="atlr-info-box">
		<h2><?php esc_html_e( 'Return Options', 'atlas-returns' ); ?></h2>
	...
	<form id="atlr-return-form" class="atlr-form">
		...
		<div class="atlr-form-row">
			<div class="atlr-form-field">
				<label for="atlr_order_id"><?php esc_html_e( 'Order ID or Phone:', 'atlas-returns' ); ?></label>
				<input type="text" id="atlr_order_id" name="order_id" required placeholder="<?php esc_attr_e( 'Enter order ID or phone number', 'atlas-returns' ); ?>" />
			</div>

			<div class="atlr-form-field">
				<label for="atlr_reason"><?php esc_html_e( 'Return Reason:', 'atlas-returns' ); ?></label>
				<select id="atlr_reason" name="reason" required>
					<option value=""><?php esc_html_e( '-- Select Reason --', 'atlas-returns' ); ?></option>
					<?php foreach ( $return_reasons as $reason_key => $reason_label ) : ?>
						<?php
						...
						<option value="<?php echo esc_attr( $reason_key ); ?>" <?php disabled( $disabled ); ?>>
							...
							<?php echo $disabled ? ' (' . esc_html__( 'Pro', 'atlas-returns' ) . ')' : ''; ?>
						</option>
					<?php endforeach; ?>
				</select>
			</div>
		...
		<div class="atlr-form-row">
			<div class="atlr-form-field">
				<label for="atlr_products_to_replace"><?php esc_html_e( 'Products to Return (SKUs):', 'atlas-returns' ); ?></label>
				<input type="text" id="atlr_products_to_replace" name="products_to_replace" placeholder="<?php esc_attr_e( 'Enter SKUs separated by commas', 'atlas-returns' ); ?>" />
				<p class="description"><?php esc_html_e( 'Enter product SKUs separated by commas (e.g., SKU001, SKU002)', 'atlas-returns' ); ?></p>
			</div>

			<div class="atlr-form-field">
				<label for="atlr_new_products"><?php esc_html_e( 'New Products (SKUs):', 'atlas-returns' ); ?></label>
				<input type="text" id="atlr_new_products" name="new_products" placeholder="<?php esc_attr_e( 'Enter SKUs separated by commas', 'atlas-returns' ); ?>" />
				<p class="description"><?php esc_html_e( 'Enter product SKUs for replacement products', 'atlas-returns' ); ?></p>
			</div>
		</div>
		...
		<div class="atlr-form-actions">
			<button type="submit" id="atlr_submit" class="button button-primary button-large" <?php disabled( ! $can_create ); ?>>
				<?php esc_html_e( 'Create Return Order', 'atlas-returns' ); ?>
			</button>
	...
	<div id="atlr-preview-section" class="atlr-preview" style="display: none;">
		<h2><?php esc_html_e( 'Order Preview', 'atlas-returns' ); ?></h2>
